    ...
    <div>
      <div style={s.hero}>
        <div style={s.heroInner}>
          ...
          <h1 style={s.heroTitle}>Find Your Dream Job<br /><span style={s.heroAccent}>with AI on Your Side</span></h1>
          <p style={s.heroSub}>TalentAI matches candidates to jobs using intelligent scoring,<br />and writes personalised cover letters that actually get read.</p>
          <div style={s.heroBtns}>
            <Link to="/jobs" className="btn btn-primary" style={{ fontSize:16,padding:'14px 32px' }}>Browse Jobs →</Link>
            {!user && <Link to="/register" className="btn btn-outline" style={{ fontSize:16,padding:'14px 32px' }}>Get Started Free</Link>}
          </div>
          <div style={s.stats}>
...
const s = {
  hero: { padding:'100px 24px 80px',textAlign:'center',background:'radial-gradient(ellipse at 50% 0%,rgba(108,99,255,0.12) 0%,transparent 70%)' },
  ...
  heroSub: { fontSize:18,color:'#888899',lineHeight:1.6,marginBottom:36 },
  heroBtns: { display:'flex',gap:16,justifyContent:'center',marginBottom:48,flexWrap:'wrap' },
  stats: { display:'flex',gap:48,justifyContent:'center' },
