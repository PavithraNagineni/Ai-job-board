...
function JobCard({ job }) {
  const salary = job.salaryMin && job.salaryMax
    ? `$${(job.salaryMin/1000).toFixed(0)}k – $${(job.salaryMax/1000).toFixed(0)}k` : 'Salary not listed';
  ...
  return (
    <Link to={`/jobs/${job._id}`} style={{ display:'block' }}>
      <div className="card" style={s.jobCard}>
        ...
        <p style={s.desc}>{job.description.slice(0,120)}...</p>
        <div style={s.skills}>
          {job.skills.slice(0,5).map(sk => <span key={sk} style={s.skill}>{sk}</span>)}
        </div>
        <div style={s.jobFooter}>
          <span style={{ color:'#22c55e',fontWeight:600,fontSize:14 }}>{salary}</span>
          <span style={{ color:'#888899',fontSize:13 }}>{daysAgo===0?'Today':`${daysAgo}d ago`} · {job.applicantCount} applicants</span>
        </div>
      </div>
    </Link>
  );
  ...
  return (
    <div className="page-container">
      ...
      {loading ? (
        ...
        <div style={s.empty}><div style={{ fontSize:48,marginBottom:16 }}>🔍</div><h3>No jobs found</h3></div>
      ) : (
        <div style={s.grid}>{jobs.map(job => <JobCard key={job._id} job={job} />)}</div>
      )}
      {pages > 1 && (
...
const s = {
  ...
  jobCard:{ transition:'all 0.2s',cursor:'pointer' },
