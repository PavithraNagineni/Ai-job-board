...
const STATUS_COLORS = { pending:'badge-yellow',reviewing:'badge-purple',shortlisted:'badge-green',rejected:'badge-red',hired:'badge-green' };
    ...
    <div style={s.overlay} onClick={onClose}>
      <div style={s.modal} onClick={e => e.stopPropagation()}>
        ...
        {loading
          ...
          ? <p style={{ color:'#888899',padding:24 }}>No applicants yet.</p>
            ...
            <div key={app._id} style={s.appRow}>
              ...
              <div style={{ display:'flex',gap:10,alignItems:'center' }}>
                ...
                <select className="form-control" value={app.status} onChange={e => handleStatus(app._id, e.target.value)}
                  ...
                  {['pending','reviewing','shortlisted','rejected','hired'].map(st => <option key={st}>{st}</option>)}
                </select>
              </div>
            </div>
          ))
  ...
  return (
    <div className="page-container">
      ...
      <div style={s.statsGrid}>
        {[
          ...
          <div key={st.label} className="card" style={s.statCard}>
            ...
            <div style={{ fontSize:36,fontWeight:800,fontFamily:'Syne,sans-serif',color:'#6c63ff',marginBottom:4 }}>{st.value}</div>
            <div style={{ color:'#888899',fontSize:14 }}>{st.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card">
        ...
        {loading
          ? <div style={{ textAlign:'center',padding:40 }}><div className="spinner" style={{ width:36,height:36 }} /></div>
          : jobs.length === 0
          ? (
              ...
              <table style={s.table}>
                <thead>
                  <tr style={{ borderBottom:'1px solid #2a2a3a' }}>
                    {['Job Title','Type','Location','Applicants','Status','Posted','Actions'].map(h => (
                ...
                <tbody>
                  {jobs.map(job => (
                    <tr key={job._id} style={{ borderBottom:'1px solid #1c1c26' }}>
                      <td style={s.td}><Link to={`/jobs/${job._id}`} style={{ color:'#e8e8f0',fontWeight:500 }}>{job.title}</Link></td>
                      <td style={s.td}><span className="badge badge-purple">{job.type}</span></td>
                      <td style={s.td}><span style={{ color:'#888899',fontSize:13 }}>{job.location}</span></td>
                      <td style={s.td}>
                        <button onClick={() => setSelected({ id:job._id,title:job.title })} style={s.appBtn}>
                          {job.applicantCount} view
                        </button>
                      </td>
                      <td style={s.td}><span className={`badge ${job.isActive?'badge-green':'badge-gray'}`}>{job.isActive?'Active':'Closed'}</span></td>
                      <td style={s.td}><span style={{ color:'#888899',fontSize:13 }}>{new Date(job.createdAt).toLocaleDateString()}</span></td>
                      <td style={s.td}>
...
const s = {
  ...
  closeBtn:{ background:'transparent',border:'none',color:'#888899',fontSize:20,cursor:'pointer' },
