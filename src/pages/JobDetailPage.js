...
export default function JobDetailPage() {
  ...
  const handleApply = async () => {
    ...
    setApplying(true);
    try {
      await applyToJob({ jobId: id, coverLetter, aiGeneratedCoverLetter: coverLetter, aiMatchScore: matchResult?.score || 0, aiMatchSummary: matchResult?.summary || '' });
  ...
  const scoreColor = (sc) => sc >= 75 ? '#22c55e' : sc >= 50 ? '#f59e0b' : '#ef4444';

  if (loading) return <div style={{ textAlign:'center',padding:80 }}><div className="spinner" style={{ width:40,height:40 }} /></div>;
  if (!job) return null;

  const salary = job.salaryMin && job.salaryMax ? `$${(job.salaryMin/1000).toFixed(0)}k – $${(job.salaryMax/1000).toFixed(0)}k/yr` : 'Salary not listed';
      ...
      <div className="card" style={{ marginBottom:24 }}>
        <div style={s.header}>
          ...
          {user?.role === 'candidate' && !applied && (
            <button className="btn btn-primary" onClick={() => setTab('apply')} style={{ flexShrink:0,padding:'12px 28px' }}>Apply Now →</button>
          )}
          ...
          <div style={{ flex:2 }}>
            ...
            {job.requirements?.length > 0 && (
              <div className="card" style={{ marginBottom:20 }}>
                ...
                <ul style={{ paddingLeft:20,color:'#ccc',lineHeight:2,fontSize:14 }}>{job.requirements.map((r,i) => <li key={i}>{r}</li>)}</ul>
              </div>
            )}
          </div>
          <div style={{ flex:1 }}>
            ...
            <div className="card">
              ...
              {[['📍','Location',job.location],['💼','Type',job.type],['📅','Experience',job.experience||'Any'],['👥','Applicants',job.applicantCount]].map(([icon,label,val]) => (
                <div key={label} style={{ display:'flex',justifyContent:'space-between',fontSize:14,marginBottom:12 }}>
                  <span style={{ color:'#888899' }}>{icon} {label}</span><span style={{ fontWeight:500 }}>{val}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'apply' && user?.role === 'candidate' && (
        <div>
          <div className="card" style={{ marginBottom:20 }}>
            ...
            <p style={{ color:'#888899',fontSize:14,marginBottom:16 }}>See how well your profile matches. Add your resume in Profile first.</p>
            {!matchResult ? (
              ...
              <div style={{ display:'flex',gap:20,alignItems:'flex-start',padding:16,background:'#1c1c26',borderRadius:10 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex',gap:20 }}>
                    <div>
                      ...
                      {matchResult.strengths?.map(st => <p key={st} style={{ fontSize:13,color:'#ccc',marginBottom:4 }}>• {st}</p>)}
                    </div>
                    <div>
          ...
          <div className="card" style={{ marginBottom:20 }}>
            ...
            <p style={{ color:'#888899',fontSize:14,marginBottom:16 }}>Generate a personalised cover letter using your resume.</p>
            <div style={{ display:'flex',gap:12,marginBottom:16,alignItems:'center' }}>
              ...
              {['professional','formal','creative'].map(t => (
                <button key={t} type="button" onClick={() => setClTone(t)}
                  style={{ padding:'5px 14px',borderRadius:6,border:`1px solid ${clTone===t?'#6c63ff':'#2a2a3a'}`,background:clTone===t?'rgba(108,99,255,0.1)':'transparent',color:clTone===t?'#6c63ff':'#888899',fontSize:13,cursor:'pointer',textTransform:'capitalize' }}>
                  {t}
                </button>
              ))}
            ...
            {coverLetter && <textarea className="form-control" value={coverLetter} onChange={e=>setCoverLetter(e.target.value)} style={{ minHeight:220,lineHeight:1.7 }} />}
          </div>
...
const s = {
  ...
  tabs:{ display:'flex',gap:4,marginBottom:24,borderBottom:'1px solid #2a2a3a' },
